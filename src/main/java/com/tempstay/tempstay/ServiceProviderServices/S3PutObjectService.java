...
public class S3PutObjectService {
    ...
    public ResponseEntity<ResponseMessage> putObjectService(String hotelownId, String key, MultipartFile image) {
        ...
        try {
            ...
            if (response.eTag().isEmpty()) {
                responseMessage.setSuccess(false);
                responseMessage
                        .setMessage("Object " + folderName + '/' + hotelownId + '/' + key + " insertion falied "
                                + response.eTag());
                return ResponseEntity.ok().body(responseMessage);
            } else {
                responseMessage.setSuccess(true);
                responseMessage
                        .setMessage(
                                preSignedURLService(hotelownId, folderName + '/' + hotelownId + '/' + key).getBody().getMessage());
            ...
            responseMessage.setSuccess(false);
            responseMessage.setMessage("Object " + folderName + '/' +hotelownId + '/' + key + " insertion falied " + e.getMessage());
            return ResponseEntity.badRequest().body(responseMessage);
        }
    }
}
